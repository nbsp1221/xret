...
`active=True` is not a guarantee that every venue operation is currently available. `timeframes` is neither an exhaustive-pagination qualification nor a verified-support claim. Market listing, historical-bar operability, and verified support are distinct facts.
